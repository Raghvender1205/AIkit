...
class Optimizer(optimizers.Optimizer):
    def __init__(self, optimizer, steps):
        ...
        aikit.utils.log.debug('Wrapping \'%s\' TF optimizer with GA of %d steps', optimizer.__class__.__name__, steps)
    
    def get_updates(self, loss, params):
        grads = self.get_gradients(loss, params)

        with K.name_scope('aikit') as name_scope:
        ...
        with hooks.GetGradients(self.optimizer, [agrad / self.steps for agrad in agrads]), \
                hooks.Update(condition=last, name_scope=name_scope),    \
                hooks.UpdateAdd(condition=last, name_scope=name_scope),    \
                hooks.UpdateSub(condition=last, name_scope=name_scope):
...
def _optimizer(optimizer):
    setattr(
        ...
        type(
            optimizer, (Optimizer,),
            {'__init__': lambda self, steps, **kwargs: Optimizer.__init__(self, optimizer=getattr(optimizers, optimizer)(**kwargs), steps=steps)}
        )
    )
