...
def parse_args():
    parser = argparse.ArgumentParser(
        description='PyTorch MNIST Example with Run:AI Elasticity')
...
def create_train_loader(args, data_dir, **kwargs):
    ...
    train_loader = torch.utils.data.DataLoader(
        dataset=train_dataset, batch_size=aikit.elastic.batch_size, shuffle=args.shuffle, **kwargs)
...
def create_optimizer(args, model):
    ...
    if random.choice([True, False]):
        ...
        optimizer = aikit.ga.torch.optim.Optimizer(
            optimizer, aikit.elastic.steps)
    else:
        # create a Run:AI GA optimizer
        optimizer = aikit.ga.torch.optim.Adadelta(
            aikit.elastic.steps, model.parameters(), lr=args.lr)
